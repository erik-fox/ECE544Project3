...
void OLED_Initialize(){
	...
	OLEDrgb_PutString(&pmodOLEDrgb_inst,"RpmTarg");
...
void ENC_Update(){
	...
	if(ticks < lastticks){//CCW Turn, decrement
		switch(Incr_Status_ROT_ENC){
			case One:
				(RPM_Target > 0) ? RPM_Target - 1 : RPM_Target - 0;
			break;
			case Five:
				(RPM_Target > 5) ? RPM_Target - 5 : RPM_Target - 0;
			break;
			case Ten:
				(RPM_Target > 10) ? RPM_Target - 10 : RPM_Target - 0;
			break;
			case Default:
				RPM_Target -= 0;
			break;
...
void OLED_Update(){
	if (ticks != lastticks) {
		...
		OLEDrgb_SetCursor(&pmodOLEDrgb_inst, 8, 2);
